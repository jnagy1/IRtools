...
% One of these strings: on, off
valid =  ischar(value) && any(strcmp(value,{'none';'dwt';'dct'}));
if ~valid
...
valid =  (ischar(value) && any(strcmpi(value,{'gmres';'fgmres';'lsqr';'cgls';'rrgmres';'cmrh';'lslu'})));
if ~valid
  errmsg = sprintf('Invalid value for OPTIONS inSolver %s: must be ''gmres'' or ''fgmres'' or ''lasq'' or ''cgls'' or ''cmrh'' or ''lslu''.',field);
else
  errmsg = '';
end
...
if ~valid
  errmsg = sprintf('Invalid value for OPTIONS inSolver %s: must be ''tv'' or ''nn'' or ''tvnn'' or ''box'' or ''spnn'' or ''none''.',field);
else
  errmsg = '';
end
