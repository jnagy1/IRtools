function [A, b, x, ProbInfo] = PRblurshake(varargin) 
% PRblurshake Image deblurring problem with random camera motion (shaking)
%
% [A, b, x, ProbInfo] = PRblurshake
% [A, b, x, ProbInfo] = PRblurshake(n)
% [A, b, x, ProbInfo] = PRblurshake(n, options)
% [A, b, x, ProbInfo] = PRblurshake(options)
%
% Generates data for use in image deblurring problems where the blur
% simulates random camera motion (shaking). Use rng to control
% reproducibility.
%
% Input:
...
%
% See also: PRblur, 
% PRblurdefocus, PRblurgauss, PRblurmotion, PRblurrotation,
% PRblurspeckle, PRdiffusion, PRinvinterp2, PRnmr, PRseismic, PRspherical,
% PRtomo, PRnoise, PRshowb, PRshowx, fspecial, rng
...
options = PRset(options, 'PSF', 'shake');
